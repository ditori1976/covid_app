...
class DataLoader:
    ...
    def __init__(self, parser: ConfigParser):
        ...
        # load geoson country data for use in mapbox
        with urlopen(parser.get("urls", "mapbox_countries_url")) as response:
            self.countries = json.load(response)

        # load country info (iso codes, population, area) from geonames
        self.country_info = self.__read_geonames_country_info(parser)

        # return definition of regions
        self.regions = self.__regions()

        # load JHU data

        # lookup table to link country names with iso codes
    ...
    def __read_geonames_country_info(self, parser):
